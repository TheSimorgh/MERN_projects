...
import { block_user, unblock_user, unfollow_user, user_private_profile, user_pub_profile } from "../../redux/features/userSlice";
import { PostStats, UserPosts } from "../../cmps";


const PublicUserProf=()=> {
  // Get the id from params
  const { userId } = useParams();
  //! Get data from store
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(user_pub_profile(userId));
  }, [userId, dispatch]);
  ...
  useEffect(() => {
    dispatch(user_private_profile());
  }, [userId, dispatch, hasBlocked, hasfollowed]);

  //Block user handler
  const blockUserHandler = () => {
    dispatch(block_user(userId));
  };

  //unBlock user handler
  const unBlockUserHandler = () => {
    dispatch(unblock_user(userId));
  };
